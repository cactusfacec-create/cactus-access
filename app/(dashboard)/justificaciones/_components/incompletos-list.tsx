...
export interface RegistroIncompletoDetectado {
  idEmpleado: string;
...
interface Props {
  incompletos: RegistroIncompletoDetectado[];
  empleados: Empleado[];
}

export function IncompletosList({ incompletos, empleados }: Props) {
    ...
    return (
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border py-10 text-center">
        ...
        <div className="flex flex-col gap-0.5">
          <p className="text-sm font-medium text-foreground">Sin registros incompletos</p>
          <p className="text-xs text-muted-foreground">
            Ningún empleado salió a almorzar sin regresar en los últimos 30 días.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-2">
      {incompletos.map((item, index) => {
        const fechaLabel = format(parseISO(item.fecha), "EEEE d 'de' MMMM", { locale: es });
        const sinJustificar = !item.justificacion || item.justificacion.estado === "rechazada";

        return (
          <div
            key={`${item.idEmpleado}-${item.fecha}`}
          ...
          >
            ...
            >
              {getInitial(item.nombreEmpleado)}
            </div>
            ...
            <div className="shrink-0">
              {item.justificacion && item.justificacion.estado !== "rechazada" ? (
                <EstadoTag j={item.justificacion} />
              ) : (
                <NuevaJustificacionDialog
                  empleados={empleados}
                  prefill={{ idEmpleado: item.idEmpleado, fecha: item.fecha, tipo: "incompleto" }}
                  trigger={
                    <button
                      type="button"
                      className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-orange-500/30 bg-orange-500/8 px-2.5 py-1 text-xs font-medium text-orange-700 transition-colors hover:bg-orange-500/15 dark:text-orange-400"
                    >
